...
@router.get("/")
async def get_expenses(current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    result = db.query("SELECT e.* FROM expenses e JOIN accounts a ON e.account_id = a.id WHERE a.user_id = ?", (current_user.id,))
    return result.to_dict(orient='records')

@router.get("/fixed-per-month")
async def get_fixed_per_month(current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    result = db.query("SELECT SUM(amount) as total_fixed FROM expenses e JOIN accounts a ON e.account_id = a.id WHERE a.user_id = ? AND e.recurrence IS NOT NULL", (current_user.id,))
    return result.to_dict(orient='records')[0]['total_fixed']

@router.get("/{id}") 
async def get_expense(id: int, current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    result = db.query("SELECT e.* FROM expenses e JOIN accounts a ON e.account_id = a.id WHERE e.id = ? AND a.user_id = ?", (id, current_user.id))
    if not result.to_dict(orient='records'):
        raise HTTPException(status_code=404, detail="Expense not found")
    return result.to_dict(orient='records')

@router.post("/")
async def create_expense(expense: Expense, current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    
    # Verify the account belongs to the user
    account = db.query("SELECT * FROM accounts WHERE id = ? AND user_id = ?", (expense.account_id, current_user.id)).to_dict(orient='records')
    if not account:
    ...
    db.execute("UPDATE accounts SET balance = balance - ?, last_updated = ? WHERE id = ? AND user_id = ?",
               (expense.amount, date.today(), expense.account_id, current_user.id))
               
    return {"message": "Expense created successfully"}

@router.put("/{id}")
async def update_expense(id: int, expense: Expense, current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    
    # Get the original expense amount and verify ownership through account
    original_expense = db.query("SELECT e.amount, e.account_id FROM expenses e JOIN accounts a ON e.account_id = a.id WHERE e.id = ? AND a.user_id = ?", (id, current_user.id))
    ...
    if original_account_id != expense.account_id:
        new_account = db.query("SELECT * FROM accounts WHERE id = ? AND user_id = ?", (expense.account_id, current_user.id)).to_dict(orient='records')
        if not new_account:
...
@router.delete("/{id}")
async def delete_expense(id: int, current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    
    # Get the expense details before deletion and verify ownership through account
    expense = db.query("SELECT e.amount, e.account_id FROM expenses e JOIN accounts a ON e.account_id = a.id WHERE e.id = ? AND a.user_id = ?", (id, current_user.id))
