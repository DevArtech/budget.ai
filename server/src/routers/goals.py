...
def create_goal(goal: Goal, current_user: Annotated[User, Depends(auth_service.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    db.execute("INSERT INTO goals (user_id, name, description, amount, date, completed, progress) VALUES (?, ?, ?, ?, ?, ?, ?)", (current_user.id, goal.name, goal.description, goal.amount, goal.deadline, int(goal.completed), goal.progress))
    return {"message": "Goal created successfully"}

@router.put("/{goal_id}")
def update_goal(goal_id: int, goal: Goal, current_user: Annotated[User, Depends(auth_service.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    db.execute("UPDATE goals SET name = ?, description = ?, amount = ?, date = ?, completed = ?, progress = ? WHERE id = ?", (goal.name, goal.description, goal.amount, goal.deadline, int(goal.completed), goal.progress, goal_id))
