...
@router.get("/")
async def get_income(current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    result = db.query("SELECT i.* FROM income i JOIN accounts a ON i.account_id = a.id WHERE a.user_id = ?", (current_user.id,))
    return result.to_dict(orient='records')

@router.get("/{id}") 
async def get_income(id: int, current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    result = db.query("SELECT i.* FROM income i JOIN accounts a ON i.account_id = a.id WHERE i.id = ? AND a.user_id = ?", (id, current_user.id))
    if not result.to_dict(orient='records'):
        raise HTTPException(status_code=404, detail="Income not found")
    return result.to_dict(orient='records')

@router.post("/")
async def create_income(income: Income, current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    
    # Verify the account belongs to the user
    account = db.query("SELECT * FROM accounts WHERE id = ? AND user_id = ?", (income.account_id, current_user.id)).to_dict(orient='records')
    if not account:
...
@router.put("/{id}")
async def update_income(id: int, income: Income, current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    
    # Get the old income amount and verify ownership through account
    old_income = db.query("SELECT i.amount, i.account_id FROM income i JOIN accounts a ON i.account_id = a.id WHERE i.id = ? AND a.user_id = ?", (id, current_user.id))
    ...
    if old_account_id != income.account_id:
        new_account = db.query("SELECT * FROM accounts WHERE id = ? AND user_id = ?", (income.account_id, current_user.id)).to_dict(orient='records')
        if not new_account:
...
@router.delete("/{id}")
async def delete_income(id: int, current_user: Annotated[UserInDB, Depends(AuthenticationService.get_current_active_user)]):
    db = BaseDatabridge.get_instance()
    
    # Get the income details before deletion and verify ownership through account
    income = db.query("SELECT i.amount, i.account_id FROM income i JOIN accounts a ON i.account_id = a.id WHERE i.id = ? AND a.user_id = ?", (id, current_user.id))
