...
class AccountsTool(BaseTool):
    ...
    def _run(self, *args, **kwargs) -> str:
        accounts = self.db.query("SELECT * FROM accounts WHERE user_id = ?", (self.user_id,))
        if accounts.empty:
...
class TransactionsPerAccountTool(BaseTool):
    ...
    def _run(self, account_id: str, *args, **kwargs) -> str:
        ...
        account_check = self.db.query("SELECT id FROM accounts WHERE id = ? AND user_id = ?", (account_id, self.user_id))
...
class SpendTool(BaseTool):
    ...
    def _run(self, *args, **kwargs) -> str:
        user = UserInDB(id=self.user_id, username="", email="", full_name="", disabled=False, hashed_password="")
        return {
            "weekly_max_budget": self.spend_service.get_budget_allotment(user), 
            "weekly_spend": self.spend_service.get_spend_over_time(user, date.today() - timedelta(days=6), date.today() + timedelta(days=1))
        }
...
class AssistantService:
    ...
    async def chat(self, user: UserInDB, message: str) -> AsyncGenerator[str, None]:
        ...
        if user.id not in self.agents:
            transactions_by_date_range_tool = TransactionsByDateRangeTool(user_id=user.id)
            ...
            system_message = SystemMessage(content=dedent(f"""
